...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor()  // Получаем Flux задач
                .flatMap(task -> task);
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().block();
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = primaryMail.take(1) // Берем только первое сообщение
                .flatMap(message -> {
    ...
    public void instant_search() {
        //todo: feel free to change code as you need
        Flux<String> userInput = userSearchInput();

        Flux<String> suggestions = userInput.switchMap(input -> autoComplete(input));
